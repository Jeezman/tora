   ...
   */
   constructor(protoPath, lndHost, lndCertPath, macaroonPath) {
    ...
    if (!fs.existsSync(lndCertPath)) {
      console.error('Required lnd certificate path missing from application configuration.');
      process.exit(1);
    }

    // default options for protoLoader.loadSync
    const options = {
      keepCase: true,
      longs: String,
      enums: String,
    ...
    this.lnrpc = grpc.loadPackageDefinition(protoLoader.loadSync(protoPath,options)).lnrpc
  ...
  static generateCredentials(lndCert, options) {
    ...
    if (options.macaroonPath) {
      if (fs.existsSync(options.macaroonPath)) {
        ...
        console.error(`The specified macaroon file ${options.macaroonPath} was not found.\n`
        ...
        if (err) {
          ...
          switch (err.code) {
            ...
            case grpc.status.UNKNOWN:
              console.debug(`gRPC API error: ${err}`);
              reject(err.message);
              break;
            default:
              console.error(`Unrecognized gRPC error: ${err}`);
